...
export default async function CheckoutSuccess({
    ...
    try {
        ...
        if (checkoutSession.status === "complete") {
            await updateUserInfoServerSide({ userInfo: { isPro: true, subscription: checkoutSession.subscription } });
        }
    } catch (err) {
    ...
    return (
        <>
            <UseUpdateSession data={{ isPro: true }} />
            <div className="flex flex-col items-center gap-4 mt-16 bg-muted/50">
                ...
                <p className="body-xl w-full px-4 text-center text-slate-11 md:w-[572px] md:px-0 text-2xl">
                    Unlock new possibilities with AI insights
                </p>
            </div>
            <div className="flex flex-col items-center my-16">
                <div className="flex flex-col items-start gap-6 mx-auto md:flex-row">
                    <div className="flex h-[353px] flex-col gap-8 rounded-lg bg-slate-2 px-6 py-12 border-[3px] border-crimson-6">
                        ...
                        <button
                            className="inline-flex gap-0.5 justify-center overflow-hidden text-sm font-medium transition rounded-[4px] bg-crimson-5 py-1 px-3 text-white hover:bg-crimson-10 w-[256px]"
                            type="button"
                        >
                            Payed Successfully
                        </button>
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <p className="body text-slate-11">Subscribe to Qashboard</p>
                            </div>
                        </div>
                    </div>
                </div>
